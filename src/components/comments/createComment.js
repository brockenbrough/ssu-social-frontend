...
const truncateUsername = (
	...
	while (
		context.measureText(`@${truncatedUsername}.. ${timeAgoString}`).width >
		maxWidth
	) {
...
function CreateComment({ post, setParentCommentCount, postCardHeight }) {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const savedComment = response.data.newComment;
	...
	function commentList() {
		...
		return comments.map((comment) => {
			...
			const truncatedUsername = truncateUsername(
				comment.username,
				timeAgoString
			);
						...
						<div className="flex space-x-1 items-start">
							...
							<div className="flex flex-col w-full">
								<div className="flex items-center">
									...
									<span className="ssu-comment-timeago">{timeAgoString}</span>
								</div>
